...
<div class="container-fluid asm-wrap">
  <div class="asm-head">
    ...
    <?php if ($action !== 'edit'): ?><a class="btn btn-primary" href="<?= asm_h(zen_href_link(FILENAME_ADVANCED_SPECIALS_MANAGER, 'action=edit')) ?>">Create rule</a><?php endif; ?>
  </div>

<?php if ($action === 'edit'): ?>
  <form method="post" action="<?= asm_h(zen_href_link(FILENAME_ADVANCED_SPECIALS_MANAGER, 'action=save')) ?>" class="asm-card">
    ...
    <div class="asm-actions"><button class="btn btn-primary" type="submit">Save rule</button><a class="btn btn-default" href="<?= asm_h(zen_href_link(FILENAME_ADVANCED_SPECIALS_MANAGER)) ?>">Cancel</a></div>
  </form>
<?php elseif ($action === 'preview' && $ruleId > 0): $preview = $manager->preview($ruleId); ?>
  <div class="asm-card">
    ...
    <div class="asm-actions">
      <form method="post" action="<?= asm_h(zen_href_link(FILENAME_ADVANCED_SPECIALS_MANAGER, 'action=apply&rule_id=' . $ruleId)) ?>"><input type="hidden" name="securityToken" value="<?= asm_h($_SESSION['securityToken'] ?? '') ?>"><button class="btn btn-success" type="submit">Apply rule</button></form>
      <a class="btn btn-primary" href="<?= asm_h(zen_href_link(FILENAME_ADVANCED_SPECIALS_MANAGER, 'action=edit&rule_id=' . $ruleId)) ?>">Edit rule</a>
      <a class="btn btn-default" href="<?= asm_h(zen_href_link(FILENAME_ADVANCED_SPECIALS_MANAGER)) ?>">Back to rules</a>
    </div>
  </div>
  ...
  <?php foreach ($preview as $product): $ready = str_starts_with((string)$product['result'], 'Ready'); ?><tr><td><?= (int)$product['products_id'] ?></td><td><?= asm_h($product['products_name'] ?: '(unnamed product)') ?></td><td><?= asm_h($product['products_model']) ?></td><td><?= asm_h(number_format((float)$product['products_price'], 4)) ?></td><td><?= $product['calculated_price'] === null ? 'Not valid' : asm_h(number_format((float)$product['calculated_price'], 4)) ?></td><td><?= asm_h($product['products_quantity']) ?></td><td><?= (int)$product['products_priced_by_attribute'] === 1 ? 'Yes' : 'No' ?></td><td class="<?= $ready ? 'asm-ready' : 'asm-conflict' ?>"><?= asm_h($product['result']) ?></td></tr><?php endforeach; ?>
...
<?php else: $rules = $manager->getRules(); ?>
  ...
  <?php foreach ($rules as $item): ?><tr><td><?= asm_h($item['rule_name']) ?></td><td><?= (int)$item['rule_status'] === 1 ? 'Enabled' : 'Disabled' ?></td><td><?= asm_h(ucfirst((string)$item['price_method'])) ?></td><td><?= asm_h($item['price_value']) ?></td><td><?= (int)$item['generated_count'] ?></td><td><a class="btn btn-xs btn-primary" href="<?= asm_h(zen_href_link(FILENAME_ADVANCED_SPECIALS_MANAGER, 'action=preview&rule_id=' . (int)$item['rule_id'])) ?>">Preview</a> <a class="btn btn-xs btn-default" href="<?= asm_h(zen_href_link(FILENAME_ADVANCED_SPECIALS_MANAGER, 'action=edit&rule_id=' . (int)$item['rule_id'])) ?>">Edit</a> <form method="post" style="display:inline" action="<?= asm_h(zen_href_link(FILENAME_ADVANCED_SPECIALS_MANAGER, 'action=delete&rule_id=' . (int)$item['rule_id'])) ?>" onsubmit="return confirm('Delete this rule and the Specials it owns?');"><input type="hidden" name="securityToken" value="<?= asm_h($_SESSION['securityToken'] ?? '') ?>"><button class="btn btn-xs btn-danger" type="submit">Delete</button></form></td></tr><?php endforeach; ?>
